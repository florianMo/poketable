...
const Board = ({ config, moment, update, pokelist, pokeIndex, initRound, playSound, }) => {
  ...
  return (
    <>
      ...
      <div style={{ ...cssPokedexWrapper(pokedexSize)}}>
        ...
        <Timer time={initRound.timer} timerFeedback={(msg)=>timerFeedback(msg)} timerActive={isStarted && !isLost} 
          style={{...positionAbsolute(7,8,13,'auto')}}
        />
        <Target pokelist={pokelist} round={round} isStarted={isStarted}
          pokeIndex={pokeIndex}
        ...
        />
        <Display round={round} riddle={riddle} lives={lives} multi={multi} isLost={isLost}
        ...
        >
          {keyboard.map((n) =>
            <motion.button
              ...
              whileTap={{
                backgroundColor: n === riddle.s ? '#22e30d': (lives>1 ? '#ffa400': '#ff4600')
              }}
